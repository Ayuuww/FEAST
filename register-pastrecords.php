...
$registrar_id = $_SESSION['idnumber'];
$faculty_id = $registrar_id; // registrar is also a faculty
$academic_year = $_GET['academic_year'] ?? '';
$semester      = $_GET['semester'] ?? '';
$subject_code  = $_GET['subject_code'] ?? '';

// --- Get list of evaluated subjects for this faculty (for the subject filter dropdown) ---
$subject_list = null;
...
if ($stmtSub) {
  ...
  error_log("Failed to prepare subject list statement: " . $conn->error);
  $subject_list = false;
}


// --- Evaluation records query (for the main table display) ---
$params = [$faculty_id];
$types = "s";
...
if ($stmt) {
  ...
  error_log("Failed to prepare main evaluation statement: " . $conn->error);
  $result = false;
}

// Get distinct academic years for the dropdown
$years_query = $conn->prepare("SELECT DISTINCT academic_year FROM evaluation WHERE faculty_id = ? ORDER BY academic_year DESC");
$years_data = [];
if ($years_query) {
  $years_query->bind_param("s", $faculty_id);
  $years_query->execute();
...
// Get distinct semesters for the dropdown, filtered by academic year if selected
$semesters_data = [];
$semester_sql = "SELECT DISTINCT semester FROM evaluation WHERE faculty_id = ?";
$semester_types = "s";
$semester_params = [$faculty_id];
      ...
      <div class="card">
        <div class="card-body table-responsive">
          ...
          <?php if ($result && $result->num_rows): ?>
            <a href="register-pastrecords-print.php?academic_year=<?= urlencode($academic_year) ?>&semester=<?= urlencode($semester) ?>&subject_code=<?= urlencode($subject_code) ?>" target="_blank" class="btn btn-outline-secondary mb-3">
