...
$stmt->bind_param("sss", $registrar_id, $selected_year, $selected_sem);
$stmt->execute();
$result = $stmt->get_result();

// ✅ Count evaluations per subject
$countQuery = "
  SELECT subject_code, COUNT(*) as total 
  FROM evaluation 
  WHERE faculty_id = ? 
    AND academic_year = ? 
    AND semester = ?
...
$countStmt->bind_param("sss", $registrar_id, $selected_year, $selected_sem);
$countStmt->execute();
...
$enrolledStmt->bind_param("sss", $registrar_id, $selected_year, $selected_sem);
