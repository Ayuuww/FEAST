...
// --- ✅ FIX 1: Fetch ONLY Colleges/Programs assigned to this Admin ---
$admin_id = $_SESSION['idnumber'];
$query_dept_prog = "
    SELECT DISTINCT a.college_name, a.program_name 
    FROM adds a
    INNER JOIN admin_college ac ON a.college_name = ac.college_name
    WHERE ac.admin_idnumber = ?
      AND a.college_name IS NOT NULL AND a.college_name != '' 
      AND a.program_name IS NOT NULL AND a.program_name != ''
    ORDER BY a.college_name, a.program_name
";

$stmt_dept = $conn->prepare($query_dept_prog);
$stmt_dept->bind_param("s", $admin_id);
$stmt_dept->execute();
$result_dept_prog = $stmt_dept->get_result();
        ...
        <div class="col-lg-6">
          <div class="card">
            <div class="card-body">
              ...
              <form class="row g-3 needs-validation" novalidate method="post" action="studentcreation.php" enctype="multipart/form-data" id="student_form">
                ...
                <div class="col-md-6">
                  <div class="form-floating">
                    <input type="text" name="first_name" class="form-control" placeholder="First Name" oninput="this.value = this.value.toUpperCase()" required>
                ...
                <div class="col-md-6">
                  <div class="form-floating">
                    <input type="text" name="mid_name" class="form-control" placeholder="Middle Name" oninput="this.value = this.value.toUpperCase()" required>
                ...
                <div class="col-md-6">
                  <div class="form-floating">
                    <input type="text" name="last_name" class="form-control" placeholder="Last Name" oninput="this.value = this.value.toUpperCase()" required>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (!hasCollegeData || !hasSectionData) {
        let missing = [];
        if (!hasCollegeData) missing.push('Colleges and Programs assigned to you');
        if (!hasSectionData) missing.push('Sections');

        Swal.fire({
          icon: 'warning',
          title: 'Missing Reference Data',
          html: `The following reference data is not found for your account: <ul><li>${missing.join('</li><li>')}</li></ul><p>Please ensure you have assigned Colleges/Programs in the system.</p>`,
